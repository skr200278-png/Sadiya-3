...
export const DuesStatusBadge: React.FC<DuesStatusBadgeProps> = ({
  ...
  if (!dueStatus.hadDueOriginally && !dueStatus.hasDueRecord) {
    return (
      <span className="text-xs font-semibold text-emerald-600 outline outline-1 outline-emerald-200 px-1.5 py-0.5 rounded mt-1 inline-block">
  ...
  if (dueStatus.isFullyPaid) {
    return (
      <div className="flex flex-col items-end gap-0.5 mt-1 text-right">
        <span className="inline-flex items-center gap-1 text-[11px] font-extrabold text-emerald-700 bg-emerald-50 border border-emerald-250 px-2 py-0.5 rounded-md shadow-2xs">
  ...
  // Scenario 3: Partially paid through Dues ledger (some still remaining)
  if (dueStatus.remainingDue > 0 && dueStatus.totalPaidInDues > 0) {
    return (
      <div className="flex flex-col items-end gap-0.5 mt-1 text-right">
        <span className="inline-flex items-center gap-1 text-[11px] font-extrabold text-amber-700 bg-amber-50 border border-amber-250 px-2 py-0.5 rounded-md shadow-2xs">
        ...
        <span className="text-[10px] text-slate-600 font-semibold tracking-tight">
          {language === 'bn' 
            ? `জমা: ৳ ${dueStatus.totalPaidInDues.toLocaleString('bn-BD')} (${formatDisplayDate(dueStatus.lastPaymentDate, language)})` 
            : `Deposited: ৳ ${dueStatus.totalPaidInDues} (${formatDisplayDate(dueStatus.lastPaymentDate, language)})`}
        </span>
        <Link 
          to="/dues" 
